...
void SystemInit(void)
{
  ...
  /* Set HSION bit */
  RCC->CR |= (uint32_t)0x00000001;  //ÄÚ²¿¸ßËÙÊ±ÖÓ´ò¿ª16M
...
/******************************************************************************/
  ...
  /* Enable HSE */
  RCC->CR |= ((uint32_t)RCC_CR_HSEON);  //´ò¿ªÍâ²¿¸ßËÙÊ±ÖÓ
 
  /* Wait till HSE is ready and if Time out is reached exit */
  do
  {
    HSEStatus = RCC->CR & RCC_CR_HSERDY; //¼ì²éÊÇ·ñ¾ÍÐ÷
    StartUpCounter++;
  } while((HSEStatus == 0) && (StartUpCounter != HSE_STARTUP_TIMEOUT));

  if ((RCC->CR & RCC_CR_HSERDY) != RESET)
  {
    HSEStatus = (uint32_t)0x01;  //¾ÍÐ÷±êÖ¾
  }
  else
  {
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    RCC->APB1ENR |= RCC_APB1ENR_PWREN; //µçÔ´½Ó¿ÚÊ±ÖÓÊ¹ÄÜ
    PWR->CR |= PWR_CR_VOS; //¼¶±ð1

    /* HCLK = SYSCLK / 1*/
    RCC->CFGR |= RCC_CFGR_HPRE_DIV1; //AHB²»·ÖÆµ

#if defined (STM32F40_41xxx) || defined (STM32F427_437xx) || defined (STM32F429_439xx)      
    /* PCLK2 = HCLK / 2*/
    RCC->CFGR |= RCC_CFGR_PPRE2_DIV2; //APB2¶þ·ÖÆµ
    
    /* PCLK1 = HCLK / 4*/
    RCC->CFGR |= RCC_CFGR_PPRE1_DIV4; //APB1ËÄ·ÖÆµ
#endif /* STM32F40_41xxx || STM32F427_437x || STM32F429_439xx */
    ...
    /* Configure the main PLL */ //PLL_M 25:¸ù¾ÝÓ²¼þ×Ô¼ºÉèÖÃ PLL_N:336 
    RCC->PLLCFGR = PLL_M | (PLL_N << 6) | (((PLL_P >> 1) -1) << 16) |
                   (RCC_PLLCFGR_PLLSRC_HSE) | (PLL_Q << 24);
//PLL_P:2  RCC_PLLCFGR_PLLSRC_HSE:Ñ¡ÔñÍâ²¿×÷ÎªPLLÊ±ÖÓÔ´ PLL_Q:7 //25/25*336/2
    /* Enable the main PLL */ 
    RCC->CR |= RCC_CR_PLLON; //PLLÊ±ÖÓ¿ªÆô
    ...
    RCC->CFGR |= RCC_CFGR_SW_PLL;//Ñ¡ÔñPLL×÷ÎªSYSCLKÊ±ÖÓÔ´

    /* Wait till the main PLL is used as system clock source */
    while ((RCC->CFGR & (uint32_t)RCC_CFGR_SWS ) != RCC_CFGR_SWS_PLL);//µÈ´ýÊ±ÖÓÔ´ÇÐ»»
    {
    }
  }
  else
