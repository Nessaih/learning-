...
uint8 HP_6_Measurement_rate(void )
{
	...
	if(data[7])  //如果可以获得心率结果，表明测量成功，则关闭心率测量
	{
		HP_6_CloseRate();
		
		Number2String(data[7],str); // //byte 7是心率结果
		...
		memset(rx_buf,0,24);
		memset(str,0,10);
		heart_rate_open_flag = 1;//重新初始化标志，以便下一次打开测量
		return 0;//测量结束
	}
	else //如果获取不到心率结果，则每隔1秒钟获取心率值
	{
		HP_6_GetRateResult();
...
uint8 HP_6_Measurement_bp(void )
{
	...
	HP_6_GetBpResult();
	HP_6_GetResultData(data);
	if(0 == data[7])                   //byte 7是血压测量的状态:0测量中，1测量完成、2测量失败
	{
	...
	else if(1 == data[7])  
	{
		...
		sprintf(str,"High:%03d        Low:%03d", data[10], data[11]);//byte 10是血压测量结果的高压,byte 11是血压测量结果的低压	
		OLED_Show_String(2,0,str,CH_SIZE2,normal);
		HP_6_CloseBp();
		memset(str,0,30);
		memset(data,0,24);
		flag = 1;//重新初始化标志，以便下一次打开测量
		return 0;//测量结束
	}		 
	else if(2 == data[7])         
	{
